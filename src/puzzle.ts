...
export class Puzzle extends Array<number> {
  ...
  protected _to2d (): number[][] {
    ...
    const _2d = []
    for (let i = 0; i < this.length; i += this.width) { _2d.push(this.slice(i, i + this.width)) }
    return (this._2d = _2d)
  }
  ...
  public get timeStarted (): number | null {
    return (this.taps.length > 0) ? this.taps[0].time : null
  }

  public get timeSolved (): number | null {
    const lastTap = this.taps.at(-1)
    return lastTap !== undefined && this.isSolved() ? lastTap.time : null
  }
  ...
  protected _checkSolvable (): boolean {
  ...
  protected _checkSolving (): boolean {
    return this.timeStarted !== null && this.timeSolved === null
  }

  protected _checkSolved (): boolean {
  ...
  public toString ({ marginWidth, marginHeight, color }: ToStringOptions = {}): string {
    ...
    this._to2d().forEach((row, pieceY) => {
      row.forEach((piece, pieceX) => {
        ...
        if (color !== undefined && piece === pieceY * this.width + pieceX) {
          repeat(gridHeight, (i) => {
  ...
  public tap (x: number, y: number): TapData | null {
    ...
    const emptyPieceX = this.getX(emptyPieceIndex)
    const emptyPieceY = this.getY(emptyPieceIndex)
    ...
    if (isSameX) {
      ...
      if (emptyPieceY < y) {
        ...
        for (; tappedPieceIndex < index; index -= this.width) { this[index] = this[index - this.width] }
      }
    } else {
      if (emptyPieceX < x) {
